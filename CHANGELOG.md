...
#### [1.0.0a2](https://github.com/hidekuma/flask-s3-viewer/compare/1.0.0a1...1.0.0a2)

> 14 May 2026 — alpha 2, pre-release.
...
- Listing search rewritten from JMESPath f-string interpolation to a Python case-insensitive `in` filter with NFC normalisation. Concrete wins:
  ...
  - **Recursive** — search switches the listing to flat (`delimiter=''`) mode scoped to the current prefix, so matches inside sub-folders are visible. Folder rows during search are synthesized for any sub-prefix whose own segment matches the query.
