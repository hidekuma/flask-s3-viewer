...
- `flask.s3viewer.core.js` comprehensive slim-down (HTMX/hyperscript replaces most of it; only the presigned-upload flow needs to remain). Currently shipped as-is for compatibility.
- mypy `strict = true` upgrade and ruff `UP` rule activation.
